# DEFINITION_OF_DONE.md — PraxisGT (Mundo Cel Diaz) — Auditoría consolidada de vendibilidad
...
**Conteo de bloqueantes: 10 CUMPLIDOS · 2 PREPARADOS (A16, B1 — esperan proveedor externo) · 2 EN PROGRESO (A12, A14).**
**Faltan para v1.0:** A12 (cookie HttpOnly — depende de subdominio propio del API) · A14 (backend de paginación listo y mergeado; falta adopción en el frontend) · A16/B1 (externos).
...
- [x] **A11 — CSP estricta** · ✅ **CUMPLIDO (29 jun)** · `vercel.json` endurecido → `script-src 'self'` (sin `unsafe-inline`/`unsafe-eval`; se mantiene `style-src 'unsafe-inline'` porque la app usa estilos inline en todos lados). PR frontend #165. **Efecto colateral detectado y corregido (PR #166):** la CSP estricta heredada por las ventanas de boleta (`window.open`+`document.write` → `about:blank`) bloqueaba el script de QR por CDN y los `<script>` inline de impresión. Fix: QR generado localmente (`src/utils/qr.js` con `qrcode-generator`, sin CDN) e impresión disparada desde el opener (`w.print()`, sin script inline) en `receipt.js`/`RepairsScreen`/`SuppliersScreen`/`CuadresScreen`. **Validado en piloto** (captura del usuario: QR renderiza en la boleta + opciones Imprimir/PDF/Descargar/WhatsApp funcionando bajo CSP estricta).
- [~] **A12 — refresh token en cookie HttpOnly** · 🟡 **EN PROGRESO** · su versión robusta depende de un **subdominio propio del API** (`api.<dominio>`) para ser cookie de primera parte; se hará al montar dominio propio.
...
- [~] **A14 — Paginación server-side `sales`/`accounts`** · 🟡 **BACKEND LISTO (mergeado a staging)** · `utils/paging.js` + paginación OPCIONAL retrocompatible en `GET /sales` y `/accounts` (`?page&limit` → `{data,total,page,limit}`; sin params → array completo como hoy, no rompe nada). +8 tests, suite 125/125. PR API #76. **Falta:** adopción en el frontend (que `History`/`Accounts` pidan por páginas con UI) — requiere validar en piloto (Vercel).
