...
**Conteo de bloqueantes: 9 CUMPLIDOS · 2 PREPARADOS (A16, B1 — esperan proveedor externo) · 2 EN PROGRESO (A11/A12, A14).**
**Faltan para v1.0:** A11 (CSP listo, falta validar en piloto cuando Vercel se libere) · A12 (cookie HttpOnly — depende de subdominio propio del API) · A14 (backend de paginación listo y mergeado; falta adopción en el frontend) · A16/B1 (externos).
...
- [~] **A11/A12 — CSP estricta + refresh token en cookie HttpOnly** · 🟡 **EN PROGRESO** · **A11 (CSP):** `vercel.json` endurecido → `script-src 'self'` (sin `unsafe-inline`/`unsafe-eval`); build verificado local; CI verde. **Falta SOLO validar en piloto** (boletas/QR/PDF/Excel) — PR frontend #165 en espera del reset de Vercel (límite diario). **A12 (cookie HttpOnly):** pendiente — su versión robusta depende de un **subdominio propio del API** (`api.<dominio>`) para ser cookie de primera parte; se hará al montar dominio propio.
