...
/* ── ProductForm ── */
var FORM_FIELDS = [
  {k:"name",    l:"Nombre",           ph:"Ej: Pantalla iPhone 11", tp:"text"  },
  ...
  return (
    <div style={Object.assign({},sC,{marginBottom:16,borderColor:TEAL,borderWidth:"1.5px"})}>
      ...
      {product.id&&product.code&&(
        <div style={{background:"#f5f4f0",borderRadius:8,padding:"8px 14px",marginBottom:12,fontSize:13,display:"flex",alignItems:"center",gap:8}}>
          <span style={{color:"#666"}}>Código:</span>
          <span style={{fontFamily:"monospace",fontWeight:700,color:TEAL,fontSize:14}}>{product.code}</span>
          <span style={{color:"#999",fontSize:11}}>(generado automáticamente)</span>
        </div>
      )}
      {!product.id&&(
        <div style={{background:"#E1F5EE",borderRadius:8,padding:"8px 14px",marginBottom:12,fontSize:12,color:"#085041"}}>
          🔢 El código se generará automáticamente al guardar (P001, P002, P003...)
        </div>
      )}
...
function App(props) {
  ...
  async function saveProduct(prod){
    ...
    if(!isNew&&isOnline){
      ...
      try{
        var saved=await productsAPI.create({name:prod.name,category:prod.category||'',shelf:prod.shelf||'',price:prod.price,cost:prod.cost||0,stock:prod.stock||0,unit:prod.unit||'uni'});
