// ══════════════════════════════════════════════════════════════════════════════
// PANTALLA: BackupScreen — Sistema de backup enterprise
//
// Muestra el estado de salud de backups, permite crear backups manuales,
// lista el historial de los últimos 30 backups y permite descargarlos.
// Los backups automáticos se ejecutan diariamente a las 2:00 AM (Guatemala).
// ══════════════════════════════════════════════════════════════════════════════
...
// Cuántas horas han pasado desde una fecha ISO
function hoursAgo(isoStr) {
  if (!isoStr) return null;
  return Math.round((Date.now() - new Date(isoStr).getTime()) / 3600000);
}

// Color del dot de estado de salud
function healthColor(hours) {
  if (hours === null) return '#ccc';
  if (hours <= 26) return '#22c55e';  // verde — dentro de las últimas 26h
  if (hours <= 50) return '#f59e0b';  // amarillo — entre 1 y 2 días
  return '#ef4444';                   // rojo — más de 2 días
}

// Badge visual por estado de backup
function StatusBadge({ status }) {
  var colorMap = { success: 'green', pending: 'amber', failed: 'red' };
  var labelMap = { success: 'Exitoso', pending: 'Pendiente', failed: 'Fallido' };
  return (
    <span style={mkBadge(colorMap[status] || 'gray')}>
      {labelMap[status] || status}
    </span>
  );
}

// Badge por tipo de backup
function TypeBadge({ type }) {
  var colorMap = { manual: 'blue', auto: 'teal' };
  var labelMap = { manual: 'Manual', auto: 'Automático' };
  return (
    <span style={mkBadge(colorMap[type] || 'gray')}>
      {labelMap[type] || type}
    </span>
  );
}
...
export default function BackupScreen() {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      var errText = err && err.error ? err.error : 'Error al crear backup';
  ...
  async function handleDownload(backup) {
    ...
    try {
      ...
      var errText = err && err.error ? err.error : 'Error al descargar';
      showMsg(errText, 'error');
    } finally {
  ...
  var healthLabel = lastSuccess
    ? (lastSuccessHours === 0
        ...
        : 'Hace ' + lastSuccessHours + ' hora' + (lastSuccessHours === 1 ? '' : 's'))
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: 24, alignItems: 'stretch' }}>
        ...
        <div style={Object.assign({}, sCard, { flex: '1 1 300px' })}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 14, fontWeight: 600, color: '#111827' }}>
              {loading ? 'Cargando…' : ('Ultimo backup exitoso: ' + healthLabel)}
            </span>
          </div>
          <div style={{ fontSize: 13, color: '#6b7280' }}>
            Proximo backup automatico: <b>Hoy a las 2:00 AM</b> (hora Guatemala)
          </div>
          {lastSuccess && (
            <div style={{ fontSize: 12, color: '#9ca3af', marginTop: 8 }}>
              Tamano: {fmtSize(lastSuccess.size_bytes)}
            </div>
          )}
        </div>
        ...
        <div style={Object.assign({}, sCard, { display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 10, flex: '0 0 auto', minWidth: 220 })}>
          <div style={{ fontSize: 13, color: '#6b7280', marginBottom: 4 }}>
            Crea un snapshot completo ahora mismo.
          </div>
          <button
            style={Object.assign({}, mkBtn('teal'), { padding: '10px 20px', fontSize: 14, opacity: creating ? 0.7 : 1, cursor: creating ? 'not-allowed' : 'pointer' })}
      ...
      <div style={Object.assign({}, sCard, { padding: 0, overflow: 'hidden' })}>
        ...
        {loading ? (
          ...
          <div style={{ padding: 32, textAlign: 'center', color: '#9ca3af', fontSize: 14 }}>
            Sin backups registrados. Crea el primero usando el boton de arriba.
          </div>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  {['Fecha / Hora', 'Tipo', 'Tamano', 'Registros', 'Estado', 'Acciones'].map(function(h) {
              ...
              <tbody>
                {pageBackups.map(function(b) {
                  var totalRecords = 0;
                  if (b.record_counts) {
                    Object.values(b.record_counts).forEach(function(n) { totalRecords += (n || 0); });
                  }
                  return (
                    <tr key={b.id} style={{ transition: 'background 0.15s' }}>
                      ...
                      <td style={sTD}>
                        {totalRecords > 0 ? (
                          <span title={b.record_counts ? JSON.stringify(b.record_counts, null, 2) : ''}>
                      ...
                      <td style={sTD}>
                        {b.status === 'success' && b.storage_path ? (
                          ...
                          b.status === 'failed' && b.error_msg ? (
                            <span style={{ fontSize: 12, color: '#ef4444' }} title={b.error_msg}>Ver error</span>
                          ) : '—'
                        )}
                      </td>
                    </tr>
        ...
        {totalPages > 1 && (
          <div style={{ padding: '12px 20px', borderTop: '1px solid rgba(0,0,0,0.06)', display: 'flex', gap: 8, alignItems: 'center', justifyContent: 'flex-end' }}>
            <button
              style={Object.assign({}, mkBtn('gray'), { fontSize: 12, opacity: page === 0 ? 0.4 : 1 })}
              onClick={function() { setPage(function(p) { return Math.max(0, p - 1); }); }}
              disabled={page === 0}
            >Anterior</button>
            <span style={{ fontSize: 13, color: '#6b7280' }}>{page + 1} / {totalPages}</span>
            <button
              style={Object.assign({}, mkBtn('gray'), { fontSize: 12, opacity: page >= totalPages - 1 ? 0.4 : 1 })}
      ...
      <div style={{ marginTop: 20, background: '#f8f9fa', borderRadius: 10, padding: '14px 18px', border: '1px solid rgba(0,0,0,0.06)' }}>
        <p style={{ margin: 0, fontSize: 13, color: '#6b7280', lineHeight: 1.6 }}>
          Los backups automaticos se realizan cada dia a las 2:00 AM (hora Guatemala).
          Se conservan los ultimos 30 dias. Cada backup incluye: clientes, productos, ventas, cuentas, reparaciones, garantias, devoluciones, proveedores, categorias, ubicaciones y configuracion de tienda. Las contrasenas nunca se incluyen en los backups.
        </p>
      </div>
    </div>
  );
