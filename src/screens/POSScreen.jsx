...
export default function POSScreen({
  ...
  function applyDiscountLocal(itemId) {
    ...
    var _it = cart.find(function(c) { return (c.serial_id || c.lineId || c.id) === itemId; });
          ...
          {cart.length === 0
            ...
            : <div style={{ flex: 1, overflowY: 'auto', marginBottom: 14 }}>
                {cart.map(function(item) {
                  var isEditing  = discountItemId === (item.serial_id || item.lineId || item.id);
                  var hasDiscount = item.originalPrice && item.price < item.originalPrice;
                  return (
                    <div key={item.serial_id || item.lineId || item.id} style={{ padding: '10px 0', borderBottom: '1px solid rgba(0,0,0,0.07)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                        ...
                        <span style={{ cursor: 'pointer', color: '#E24B4A', fontSize: 18, lineHeight: 1, flexShrink: 0 }} onClick={function() { removeFromCart(item.serial_id || item.lineId || item.id); }}>×</span>
                      </div>

                      {/* Controles de cantidad y precio */}
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <div style={sQB} onClick={function() { changeQty(item.serial_id || item.lineId || item.id, -1); }}>−</div>
                      ...
                      {!isEditing ? (
                        <div style={{ marginTop: 6, textAlign: 'right' }}>
                          <span
                            onClick={function() { setDiscountItemId(item.serial_id || item.lineId || item.id); setDiscountVal(item.price.toFixed(2)); }}
                        ...
                        <div style={{ marginTop: 8, display: 'flex', gap: 6, alignItems: 'center' }}>
                          <div style={{ flex: 1 }}>
                            ...
                            <input
                              ...
                              onKeyDown={function(e) {
                                if (e.key === 'Enter') applyDiscountLocal(item.serial_id || item.lineId || item.id);
                          ...
                          <div style={{ display: 'flex', gap: 4, marginTop: 14 }}>
                            <button style={Object.assign({}, mkBtn('teal'), { padding: '4px 8px', fontSize: 11 })} onClick={function() { applyDiscountLocal(item.serial_id || item.lineId || item.id); }}>✓</button>
