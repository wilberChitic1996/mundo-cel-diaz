...
- [x] **🔬 AUDITORÍA DE FLUJOS END-TO-END (12 jul 2026, 13 agentes por flujo de negocio):** ronda posterior a la integral — auditó FLUJOS completos (venta→caja→cuadre, crédito→abono→devolución, sesión/refresh, import, cuaderno) en vez de pantallas sueltas. **Corregido (API `claude/flujo-fixes`):** (1) abono ahora usa el SALDO autoritativo (`balance`) en vez de recomputar `total−paid` — las deudas del cuaderno (paid>0 sin account_payments) ya no se inflan al abonar; (2) devolución "Crédito en cuenta" ahora SÍ reduce la deuda real (baja total y balance del principal; paid intacto para no fabricar ingreso en Cuadres). **Corregido (frontend `claude/flujo-fixes-fe`):** JWT refrescado no llegaba a axios (sync a `mnpos-api-session`); riesgo de venta duplicada por refresh dentro del try crítico (create aislado, refresh aparte); showFlash era invisible fuera del POS (banner global); caja: efectivo contado "0" ya cuenta como arqueo real; errores de gastos ya se muestran; botones de caja ocultos para auditor (403 server-side); boleta de reparación distingue "Costo final cobrado" vs estimado; boletas históricas usan IVA guardado de la venta + línea "No reembolsado" en boleta con devolución parcial; import Excel manda `min_stock` y reporta si el stock inicial falló; cuaderno avisa "⚠ ya tiene deuda del cuaderno" en la previsualización; modal de seriales muestra a quién y cuándo se vendió el IMEI. Coherencia inter-pantallas: LIMPIA (0 hallazgos).
- [x] **Medias-features (12 jul 2026) — HECHAS v1 con aprobación del usuario (`claude/medias-features`, `en validación en piloto`):** (a) 🔎 **búsqueda global de IMEI** — botón "Buscar IMEI" en Productos, modal que consulta `GET /serials/search` y muestra producto, estado y a quién/cuándo se vendió; (b) 🎨 **selector de variante en POS** — al agregar un producto CON variantes pregunta cuál (color/capacidad), la línea del carrito lleva el nombre y precio de la variante y su propia identidad (`lineId` — no rompe seriales ni descuentos); el stock POR variante sigue sin descontarse (pendiente de diseño; el stock del producto sí se descuenta normal); (c) 🛡️ **reclamo de garantía con proceso** — al reclamar aparecen botones "↩ Hacer devolución" (abre Devoluciones con el cliente prellenado) y "🔧 Mandar a reparación" (abre Taller filtrado por cliente); (d) 💰 **columna "Diferencia"** en el historial de sesiones de caja (verde sobrante / rojo faltante).
- [x] **Ronda backlog (12 jul 2026, `claude/backlog-round`):** (1) ✅ verificado: `account_payments.idempotency_key` EXISTE en ambas BDs y la ruta ya la usaba — se cerró el círculo haciendo que Cuentas REUSE la llave en reintentos (nueva solo al cambiar el monto o tras éxito); (2) ✅ **stock por variante** ahora SÍ se descuenta al vender (líneas con `variant_id`; informativo, no revierte la venta si falla; la verdad del inventario sigue siendo `products.stock` atómico) y el selector del POS muestra las unidades disponibles por variante (caché se limpia tras cada venta).
- [x] **Ronda de diseño (12 jul 2026, `claude/design-round[-fe]`, migración 029 aditiva):** (a) 💵 **desglose de billetes en el cierre de caja** — sección opcional "Contar billetes y monedas" (Q200…Q0.25) que autocompleta el efectivo contado, se guarda en `caja_sesiones.efectivo_desglose` (JSONB) y sale en el comprobante; (b) 🎨 **devoluciones reingresan stock por variante** — `sale_items`/`return_items` ahora llevan `variant_id` y el servidor suma de vuelta a la variante; (c) 📱 **el IMEI devuelto se libera** — `sale_items`/`return_items` llevan `serial_id`; buen estado → serial vuelve a 'disponible', defectuoso → 'defectuoso' (solo ventas NUEVAS que ya guardan el serial en el ítem; ventas viejas no tienen el dato); (d) 🔗 **client_id en ventas y devoluciones** — el POS y Devoluciones mandan el id del cliente (columna TEXT por el desajuste text/uuid entre ambientes); renombrar un cliente ya no desliga su historial nuevo. Migración 029 aplicada en staging; aplicar en producción al liberar.
