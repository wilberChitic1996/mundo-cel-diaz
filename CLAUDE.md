...
- [ ] **Medias-features detectadas (12 jul — decidir alcance con el usuario antes de construir):** (a) búsqueda global de IMEI (backend `GET /serials/search` listo, sin UI); (b) selector de variante en POS (variantes hoy solo informativas; ligado al pendiente de stock por variante); (c) reclamo de garantía no dispara ningún proceso (no liga devolución/reparación); (d) historial de sesiones de caja sin columna "Diferencia"; (e) verificar si `account_payments` ya tiene columna `idempotency_key` (contradicción entre nota pendiente y hallazgo de agente).
