...
- [ ] **Backlog de auditoría (diseño pendiente, no urgente):** (a) persistir desglose de efectivo del cierre de caja en el servidor para el comprobante; (b) reingresar stock POR VARIANTE en devoluciones (los ítems de devolución no llevan variant_id); (c) liberar serial al devolver un producto con IMEI (requiere saber qué serial se devolvió); (d) `client_id` en sales/returns para que renombrar cliente no desligue su historial.
